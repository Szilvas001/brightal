...
router.post('/order',auth.requireUser,(req,res)=>{
 const run=async()=>{
  ...
  if(held){if(held.userId===req.user.id&&held.status==='approved')return res.json({request:R.toPublic(held)});return res.status(409).json({error:'STONE_RESERVED'});}
  const request=R.build({user:req.user,body:{},files:[],lang:req.body.lang});
  request.diamond=D.publicStone(stone);request.price=request.diamond.price;request.status='approved';request.approvedAt=new Date().toISOString();
