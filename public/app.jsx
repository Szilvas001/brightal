...
function AdminCad(){
 const [source,setSource]=useState('own'),[requests,setRequests]=useState([]),[config,setConfig]=useState(null),[parameters,setParameters]=useState(null),[schema,setSchema]=useState(null),[job,setJob]=useState(null),[message,setMessage]=useState('');
 useEffect(()=>{api('/admin/cad/schema').then(setSchema).catch(()=>{});api('/admin/requests?source=designer').then(x=>setRequests(x.requests)).catch(()=>{});},[]);
 useEffect(()=>{if(!job||!['queued','running'].includes(job.status))return;const timer=setInterval(()=>api('/admin/cad/jobs/'+job.id).then(setJob).catch(()=>setMessage('Az állapot nem tölthető be.')),1500);return()=>clearInterval(timer);},[job?.id,job?.status]);
 const load=async()=>{try{const own=source==='own'?JSON.parse(localStorage.getItem('brightal-design-v1')||'null'):null;const c=source==='own'?(own?.design||own?.config||own):requests.find(x=>x.id===source)?.design?.config;if(!c)throw Error('Előbb ments egy saját tervet a gyűrűtervezőben, vagy válassz rendelést.');const result=await api('/admin/cad/parameters',{method:'POST',body:source==='own'?{config:c}:{requestId:source}});setConfig(c);setParameters(result.parameters);setJob(null);setMessage('');}catch(e){setMessage(e.message);}};
 ...
 return <details className="diamond-import"><summary>Admin CAD-műhely · STEP / STL</summary><p>Gyártás-előkészítés. Az ötvös végső műszaki jóváhagyása szükséges. A STEP szerkeszthető testgeometriát, a ZIP a parametrikus generátort is tartalmazza.</p><label>Konfiguráció<select value={source} onChange={e=>{setSource(e.target.value);setParameters(null);}}><option value="own">Saját mentett terv</option>{requests.map(r=><option key={r.id} value={r.id}>{r.requestNumber} · {r.design?.name||r.design?.config.style}</option>)}</select></label><button className="btn btn-ghost" onClick={load}>Műszaki paraméterek megnyitása</button>{parameters&&<><p>Belső kerület: {(parameters.innerDiameter*Math.PI).toFixed(2)} mm · {parameters.style}</p><div className="cad-fields">{Object.entries(schema?.limits||{}).filter(([key])=>!(['prongCount','prongDiameter','prongRotation'].includes(key)&&parameters.setting!=='claw')&&!(key==='sideCount'&&parameters.style!=='contour')&&!(key==='sideSize'&&!['contour','trilogy'].includes(parameters.style))&&!(key==='gap'&&parameters.style!=='openpair')&&!(key==='sculpt'&&!['contour','curvedoval','wavebezel'].includes(parameters.style))).map(([key,[min,max]])=><label key={key}>{labels[key]}<input type="number" min={min} max={max} step={['prongCount','sideCount'].includes(key)?1:.01} value={parameters[key]} onChange={e=>setParameters({...parameters,[key]:Number(e.target.value)})}/></label>)}{[['profile',['round','flat','knife']],['setting',['bezel','claw']],['orientation',['north','east']]].map(([key,options])=><label key={key}>{key}<select value={parameters[key]} onChange={e=>setParameters({...parameters,[key]:e.target.value})}>{options.map(x=><option key={x}>{x}</option>)}</select></label>)}</div><button className="btn btn-dark" disabled={['queued','running'].includes(job?.status)} onClick={async()=>{try{const {stones,...overrides}=parameters;setJob(await api('/admin/cad/jobs',{method:'POST',body:{...(source==='own'?{config}:{requestId:source}),overrides}}));setMessage('');}catch(e){setMessage(e.message);}}}>CAD generálása és ellenőrzése</button></>}{job&&<div role="status"><p>{({queued:'Várakozik',running:'CAD generálása…',ready:'Export elkészült',failed:'Geometriai ellenőrzés / generálás sikertelen'})[job.status]}</p>{job.error&&<p>{job.error}</p>}{job.report&&<p>{job.report.solidCount} fémtest · zárt: {String(job.report.closedShells)} · STEP-visszaolvasás: {String(job.report.stepRoundtrip||false)}<br/>{job.report.warning}</p>}{job.status==='ready'&&<div className="cad-actions">{['step','stl','3mf','json','zip'].map(format=><button className="btn btn-ghost" key={format} onClick={async()=>{try{const r=await api('/admin/cad/jobs/'+job.id+'/download',{method:'POST',body:{format}});location.href=r.url;}catch(e){setMessage(e.message);}}}>{format.toUpperCase()} letöltés</button>)}</div>}</div>}<p role="alert">{message}</p><small>Támogatott CAD-családok: {schema?.styles.join(', ')}. Más családoknál külön CAD-előkészítés szükséges; a rendszer nem helyettesíti őket eltérő modellel.</small></details>;
}
