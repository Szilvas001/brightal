...
function DiamondArtwork({stone}) {
 const [ready,setReady]=useState(false),[failed,setFailed]=useState(false);
 return <div className={'diamond-preview '+(ready?'ready':'')}>
 {!ready&&!failed&&<div className="diamond-skeleton" aria-label="Termékkép betöltése"/>}
 {failed?<span className="diamond-image-fallback">{stone.shape} · {stone.carat} ct<br/>360° nézet megnyitása ↗</span>:<img src={stone.previewUrl} width="640" height="520" loading="lazy" decoding="async" alt={`${stone.shape} csiszolású gyémánt, ${stone.length} × ${stone.width} × ${stone.height} mm – 3D szemléltetés`} onLoad={()=>setReady(true)} onError={()=>setFailed(true)}/>}
 </div>;
}
...
function DiamondsPage() {
 ...
 const filtered=(data?.items||[]).filter(x=>{
   ...
   for(const key of ['carat','price','ratio','depth','table']){if(x[key]==null&&(q[key+'Min']||q[key+'Max']))return false;if(q[key+'Min']!==undefined&&q[key+'Min']!==''&&x[key]<Number(q[key+'Min']))return false;if(q[key+'Max']!==undefined&&q[key+'Max']!==''&&x[key]>Number(q[key+'Max']))return false;}return true;
 }).sort((a,b)=>q.sort==='desc'?b.price-a.price:a.price-b.price);
 const order=async()=>{if(!user){setAuthModal('login');close();return;}setBusy(true);setError('');try{const {request}=await api('/diamonds/order',{method:'POST',body:{id:selected.id,expectedPrice:selected.price,acceptTerms:terms,lang}});setCart(cart.filter(id=>id!==selected.id));const payment=await api('/payment/start',{method:'POST',body:{requestNumber:request.requestNumber}});location.href=payment.gatewayUrl;}catch(e){setError(({PRICE_CHANGED:'Az ár megváltozott. Frissítsd a katalógust a vásárlás előtt.',PRICE_UNAVAILABLE:'Az ár ellenőrzése folyamatban van.',STONE_RESERVED:'Ez a kő már foglalt.',STONE_NOT_PURCHASABLE:'Ez a kő nem rendelhető.'})[e.code]||'A fizetés nem indult el. A létrejött rendelést a fiókodban is megtalálod.');}finally{setBusy(false);}};
...
function AdminCad(){
 const [source,setSource]=useState('own'),[requests,setRequests]=useState([]),[library,setLibrary]=useState([]),[jobs,setJobs]=useState([]),[config,setConfig]=useState(null),[parameters,setParameters]=useState(null),[schema,setSchema]=useState(null),[job,setJob]=useState(null),[message,setMessage]=useState('');
 const refresh=()=>{try{setLibrary(JSON.parse(localStorage.getItem('brightal-library-v2')||'[]'));}catch{setLibrary([]);}api('/admin/cad/jobs').then(x=>setJobs(x.jobs)).catch(()=>setMessage('Az exportlista nem tölthető be.'));};
 useEffect(()=>{api('/admin/cad/schema').then(setSchema).catch(()=>{});api('/admin/requests?source=designer').then(x=>setRequests(x.requests)).catch(()=>{});refresh();window.addEventListener('storage',refresh);return()=>window.removeEventListener('storage',refresh);},[]);
 useEffect(()=>{if(!job||!['queued','running'].includes(job.status))return;const timer=setInterval(()=>api('/admin/cad/jobs/'+job.id).then(j=>{setJob(j);if(!['queued','running'].includes(j.status))refresh();}).catch(()=>setMessage('Az állapot nem tölthető be.')),1500);return()=>clearInterval(timer);},[job?.id,job?.status]);
 const selected=()=>{if(source==='own'){const own=JSON.parse(localStorage.getItem('brightal-design-v1')||'null');return own?.design||own?.config||own;}if(source.startsWith('saved:'))return library.find(x=>String(x.id)===source.slice(6))?.design;return requests.find(x=>x.id===source)?.design?.config;};
 const load=async()=>{try{const c=selected();if(!c)throw Error('Előbb ments egy tervet a gyűrűtervezőben, majd frissítsd a listát.');setConfig(c);setParameters(null);setJob(null);setMessage('');if(schema?.styles.includes(c.style)){const result=await api('/admin/cad/parameters',{method:'POST',body:{config:c}});setParameters(result.parameters);}}catch(e){setMessage(e.message);}};
 const generate=async mode=>{try{const c=config||selected();if(!c)throw Error('Válassz mentett tervet.');const body={config:c,name:library.find(x=>'saved:'+x.id===source)?.name||c.style};if(mode==='cad'){const {stones,...overrides}=parameters;body.overrides=overrides;}setJob(await api('/admin/cad/'+(mode==='mesh'?'mesh-jobs':'jobs'),{method:'POST',body}));setMessage('');}catch(e){setMessage(e.message);}};
 const download=async(j,format)=>{try{const r=await api('/admin/cad/jobs/'+j.id+'/download',{method:'POST',body:{format}});location.href=r.url;}catch(e){setMessage(e.message);}};
 const labels={innerDiameter:'Belső átmérő (mm)',width:'Sínszélesség',thickness:'Sínvastagság',stoneLength:'Kő hossza',stoneWidth:'Kő szélessége',stoneDepth:'Kő mélysége',bezelWall:'Bezel falvastagság',prongDiameter:'Karom átmérő',prongCount:'Karmok száma',prongRotation:'Karmok szöge',underOpening:'Kő alatti nyílás',settingHeight:'Foglalatmagasság',sideSize:'Mellékkő mérete',sideCount:'Mellékkövek száma',minimumWall:'Minimális anyagvastagság',allowance:'Gyártási ráhagyás',tolerance:'Foglalási hézag',sculpt:'Íveltség',gap:'Nyitott rész'};
 return <details className="diamond-import" open><summary>Admin CAD-műhely · mentett tervek és letöltések</summary><p>Minden mentett modell pontos 3D terve exportálható. A támogatott családokhoz külön parametrikus STEP is készíthető.</p><label>Mentett gyűrűtervek<select value={source} onChange={e=>{setSource(e.target.value);setConfig(null);setParameters(null);setJob(null);setMessage('');}}><option value="own">Legutóbb mentett terv</option><optgroup label="Saját gyűjtemény">{library.map(x=><option key={x.id} value={'saved:'+x.id}>{x.name} · {x.design?.style}</option>)}</optgroup><optgroup label="Vásárlói rendelések">{requests.map(r=><option key={r.id} value={r.id}>{r.requestNumber} · {r.design?.name||r.design?.config.style}</option>)}</optgroup></select></label><div className="cad-actions"><button className="btn btn-ghost" onClick={refresh}>Lista frissítése</button><button className="btn btn-ghost" onClick={load}>Terv megnyitása</button><button className="btn btn-dark" disabled={['queued','running'].includes(job?.status)} onClick={()=>generate('mesh')}>3D terv elkészítése · STL / OBJ / ZIP</button></div>{config&&!schema?.styles.includes(config.style)&&<p>Ehhez a modellhez az eredeti gyűrűgeometria tölthető le OBJ-ként, validált fémtest esetén STL-ként is. Parametrikus STEP ehhez a családhoz még nincs.</p>}
 {parameters&&<details><summary>Parametrikus STEP · műszaki beállítások</summary><div className="cad-fields">{Object.entries(schema?.limits||{}).map(([key,[min,max]])=><label key={key}>{labels[key]}<input type="number" min={min} max={max} step={['prongCount','sideCount'].includes(key)?1:.01} value={parameters[key]} onChange={e=>setParameters({...parameters,[key]:Number(e.target.value)})}/></label>)}</div><button className="btn btn-dark" disabled={['queued','running'].includes(job?.status)} onClick={()=>generate('cad')}>STEP generálása és ellenőrzése</button></details>}
 {job&&<div role="status"><p>{({queued:'Várakozik…',running:'3D export készül…',ready:'A letölthető fájlok elkészültek.',failed:'Az export nem készült el.'})[job.status]}</p>{job.error&&<p>{job.error}</p>}{job.report&&<p>{job.report.warning}</p>}{job.status==='ready'&&<div className="cad-actions">{job.formats?.map(f=><button className="btn btn-dark" key={f} onClick={()=>download(job,f)}>{f.toUpperCase()} letöltés</button>)}</div>}</div>}
 <label>Korábban elkészült exportok<select value={job?.id||''} onChange={e=>{setJob(jobs.find(j=>j.id===e.target.value)||null);setMessage('');}}><option value="">Válassz elkészült 3D tervet</option>{jobs.map(j=><option key={j.id} value={j.id}>{j.source?.name||j.source?.config?.style||'Gyűrű'} · {new Date(j.createdAt).toLocaleString('hu-HU')} · {j.status}</option>)}</select></label><p role="alert">{message}</p><small>A saját gyűjtemény ebben a böngészőben tárolódik. Az elkészült exportok a szerveren maradnak. Az ötvös végső műszaki jóváhagyása szükséges.</small></details>;
}

function DiamondPriceAdmin(){
