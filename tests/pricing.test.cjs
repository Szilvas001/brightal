...
test('fixed-point gross discount with HALF_UP ties and VAT reconciliation',()=>{
 for(const [input,expected] of [['100000',90000],['123455',111110],['5',5],['100000.55',90000],['100000.56',90001],['9999999999.99',9000000000]])assert.equal(P.price(input),expected);
...
test('checkout blocks stale prices, tampered totals and failed verification',()=>{
 const stone={referencePrice:'123455',priceCheckedAt:new Date().toISOString(),sourceStatus:'verified',referenceSource:'noordia',referenceVatIncluded:true,available:true,certificate:{number:'TEST'}};
 assert.equal(P.checkout(stone,111110).price,111110);
 assert.throws(()=>P.checkout(stone,1),/PRICE_CHANGED/);
 assert.throws(()=>P.checkout({...stone,sourceStatus:'failed'},111110),/PRICE_UNAVAILABLE/);
 assert.throws(()=>P.checkout({...stone,priceCheckedAt:'2020-01-01'},111110),/PRICE_UNAVAILABLE/);
});
