...
test('every CAD endpoint rejects anonymous and non-admin direct requests',async()=>{
 ...
 try{for(const role of [null,'user'])for(const [method,url] of [['GET','/schema'],['POST','/parameters'],['POST','/jobs'],['GET','/jobs/abc'],['POST','/jobs/abc/download'],['GET','/download/guess']]){const result=await fetch(base+url,{method,headers:role?{'x-test-role':role}:{}});assert.equal(result.status,403,method+url);}
