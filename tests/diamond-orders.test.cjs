...
test('authenticated order, price tampering, ownership and concurrent reservation',async()=>{
 ...
 D.catalogue=()=>[{...D.validate(D.sample())[0],id:'unit-test-stone',demo:false,certificate:{lab:'IGI',number:'TEST_ONLY_NOT_A_REPORT',url:'https://www.igi.org/'}}];
 ...
 try{
  ...
  const [a,b]=await Promise.all([post('a',{id:'unit-test-stone',acceptTerms:true,price:1}),post('b',{id:'unit-test-stone',acceptTerms:true})]);assert.equal(a.status,200);assert.equal(b.status,409);assert.equal(rows.length,1);const result=await a.json();assert.equal(result.request.price,144000);assert.equal(result.request.source,'diamond');assert.equal(result.request.payable,true);
  assert.equal((await post('a',{id:'unit-test-stone',acceptTerms:true})).status,200);assert.equal(rows.length,1);
