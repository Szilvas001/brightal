...
test('payment rechecks reference freshness and exact gross before calling the gateway',async()=>{
 ...
 let stone={id:'test',available:true,referencePrice:'123455',referenceSource:'noordia',referenceVatIncluded:true,priceCheckedAt:new Date().toISOString(),sourceStatus:'verified',certificate:{number:'TEST'}};
 const r={id:'r',userId:'u',requestNumber:'TEST',status:'approved',price:111110,currency:'HUF',diamond:{id:'test'},history:[],customer:{},accounting:P.accounting(111110)};
 ...
 try{assert.equal((await pay()).status,200);assert.equal(charged,111110);assert.equal(invoice.buildInvoiceData(r).item.gross,111110);assert.equal(invoice.buildInvoiceData(r).item.vat,Number(P.accounting(111110).vatHuf));charged=null;stone={...stone,referencePrice:'200000'};assert.equal((await pay()).status,409);assert.equal(charged,null);stone={...stone,priceCheckedAt:'2020-01-01'};assert.equal((await pay()).status,409);assert.equal(charged,null);}
